...
def test_synthetic(generate_data, p1, p2, p3, q1, n_chains, n_freq, n_conf):
    ...
    return freqs, confs, times


def tostring(x):
    """Convert an itemset to a string."""
    res = []
    item, c = x
...
if __name__ == "__main__":
    ...
    freqs, confs, times = test_synthetic(load_data, p1, p2, p3, q1, n_chains, 300, 5)

    # plot the running time of different M
    plt.plot(n_chains, times)
    plt.xlabel("number of samples")
    plt.ylabel("running time (s)")
    # plt.savefig('running_time_m.png', bbox_inches='tight')
    plt.show()

    # plot the estimated frequency
    ...
    # plot the estimated confidence
    conf_true = {x1: [1] * len(n_chains),
                 x2: [1] * len(n_chains),
                 x3: [1] * len(n_chains),
                 x4: [q1] * len(n_chains)}
    for x in confs:
